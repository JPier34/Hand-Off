...
let _amount:      bigint       = parseEther('0.633')
let _description: string       = 'iPhone 14 Pro – Facebook Marketplace'
...
export function mockCancel()   { _status = EscrowStatus.CANCELED }
export function mockExpire()   { _expiresAt = BigInt(Math.floor(Date.now() / 1000) - 60) }
export function mockEditDeal(amount: bigint, description: string) {
  _amount = amount
  _description = description
}
export function resetMock()    { _status = EscrowStatus.PENDING; _buyer = ZERO; _expiresAt = BigInt(Math.floor(Date.now() / 1000) + 86400 * 7); _amount = parseEther('0.05'); _description = 'iPhone 14 Pro – Facebook Marketplace' }
